...
/* Write a byte to the PGC command buffer. If the buffer overflows, wait until
 * the PGC has processed some bytes before continuing. */
void pgc_write(byte b)
{
	gl_pgc[IN_WRPTR] = b;

	/* If buffer is now full, wait until the PGC catches up */
	while ((IN_WRPTR + 1) == IN_RDPTR);
	++IN_WRPTR;
}
...
/* Write a hex command to the PGC. This is like the ASCII version, but does
 * not stop at a zero byte. */
void pgc_xcmd(const byte *s, int count)
{
	while (count)
	{
		pgc_write(*s);
...
void pgc_xsay(const byte *s, int count)
{
	int ch, n;
...
/* Switch between ASCII and Hex mode. */
void hexmode(int hex)
{
	if (hex)
	{
		pgc_say("CX\n");
...
/* monsterbuf now holds the IW command. Execute it. */
		pgc_xsay(monsterbuf, gl_monstercount);
		gl_monstercount = 0;
